...
export default function ServiciosDashboardPage() {
  ...
  return (
    <div className="max-w-5xl mx-auto p-4 lg:p-6">
      ...
      <p className="text-sm text-zinc-500 mb-5">Dashboard de gastos y vencimientos</p>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 mb-6">
        ...
        >
          <div className="space-y-0.5">
            {gastoMesARS > 0 && <p className="text-sm font-semibold text-zinc-100">{formatARS(gastoMesARS)}</p>}
            {gastoMesUSD > 0 && <p className="text-sm font-semibold text-zinc-100">{formatUSD(gastoMesUSD)}</p>}
            {gastoMesARS === 0 && gastoMesUSD === 0 && <p className="text-sm text-zinc-500">—</p>}
          </div>
        </KPICard>
        ...
        >
          <p className="text-sm text-zinc-500">Pendiente integración</p>
        </KPICard>
        ...
        >
          <p className="text-sm font-semibold text-zinc-100">
            {vencidos.length}
            {vencidoMonto > 0 && <span className="text-xs text-zinc-500 font-normal ml-1">≈ {formatUSD(vencidoMonto)}</span>}
          </p>
        </KPICard>
        ...
        >
          <p className="text-sm font-semibold text-zinc-100">
            {pendientes.length}
            {pendienteMonto > 0 && <span className="text-xs text-zinc-500 font-normal ml-1">≈ {formatUSD(pendienteMonto)}</span>}
          </p>
        </KPICard>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2">
          <h2 className="text-sm font-semibold text-zinc-300 mb-3">Próximos vencimientos</h2>
          {upcoming.length === 0 ? (
            <div className="rounded-2xl border border-white/[0.06] p-8 text-center text-zinc-600 text-sm">
        ...
        <div>
          <h2 className="text-sm font-semibold text-zinc-300 mb-3">Calendario</h2>
          <MiniCalendar />
        </div>
      </div>
