      ...
      >
        ...
        <div className="p-4 space-y-4">
          ...
          <div className="relative" onClick={(e) => e.stopPropagation()}>
            <label className="text-xs md:text-[11px] text-text-muted font-medium block mb-1">Nombre del contacto *</label>
            <div className="relative flex items-center">
              <input
                ...
                className="w-full bg-surface-overlay rounded-xl px-3 py-2.5 text-sm text-text-primary outline-none border border-border-subtle focus:border-blue-500/30 disabled:opacity-60"
              />
              {searchQuery && (
                <button
                  type="button"
            ...
            {showSuggestions && searchQuery.trim() && !selectedContactId && (
              <div className="absolute left-0 right-0 mt-1 bg-[#1e1e2d] border border-border-default rounded-xl shadow-2xl z-50 max-h-60 overflow-y-auto divide-y divide-white/[0.04]">
                {filteredContacts.length === 0 ? (
                  <div className="p-3 text-xs text-text-muted text-center">
                    No se encontraron contactos.
                    <p className="mt-1 text-xs md:text-[10px] text-text-muted">Creá el contacto en la sección de contactos primero.</p>
                  </div>
                ) : (
                  filteredContacts.map(c => (
                    <button
                      ...
                      className="w-full text-left px-3 py-2.5 text-xs text-text-secondary hover:bg-surface-overlay-hover hover:text-white transition-colors flex flex-col gap-0.5 cursor-pointer"
                    >
                      <span className="font-semibold">{c.first_name} {c.last_name ?? ''}</span>
                      <span className="text-text-muted text-xs md:text-[10px]">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="text-xs md:text-[11px] text-text-muted font-medium block mb-1">Teléfono *</label>
              <input
                ...
                className="w-full bg-surface-overlay rounded-xl px-3 py-2.5 text-sm text-text-primary outline-none border border-border-subtle focus:border-blue-500/30 disabled:opacity-50"
              />
            </div>
            <div>
              <label className="text-xs md:text-[11px] text-text-muted font-medium block mb-1">Email</label>
              <input
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <select
                ...
                className="w-full bg-[#1a1a24] rounded-xl px-3 py-2.5 text-sm text-text-secondary outline-none border border-border-subtle cursor-pointer [color-scheme:dark]"
              >
                {pipelines.map(p => (
            ...
            <div>
              <label className="text-xs md:text-[11px] text-text-muted font-medium block mb-1">Fase</label>
              <select
          ...
          <div>
            ...
            <textarea
              ...
              disabled={!selectedContactId}
              rows={2}
              className="w-full bg-surface-overlay rounded-xl px-3 py-2.5 text-sm text-text-primary placeholder:text-zinc-700 outline-none resize-none border border-border-subtle focus:border-blue-500/30 disabled:opacity-50"
            />
          </div>
        </div>
