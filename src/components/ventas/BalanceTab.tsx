  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <h3 className="text-lg font-bold text-shell-text">Balance Anual {selectedYear}</h3>
          <p className="text-sm text-shell-text-muted">Desempeño mensual por asesor</p>
        </div>
        <div className="flex items-center gap-2">
          ...
          <button onClick={fetchBalance} className="p-2 rounded-xl bg-shell-surface border border-shell-border text-shell-text-muted hover:text-shell-text transition-colors cursor-pointer">
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left border-collapse min-w-[800px]">
            ...
            <tbody>
              {isLoading ? (
                <tr>
                  <td colSpan={14} className="p-8 text-center text-sm text-shell-text-muted">
                ...
                <tr>
                  <td colSpan={14} className="p-12 text-center text-sm text-shell-text-muted">
                    <div className="w-14 h-14 rounded-full bg-white/5 flex items-center justify-center mb-3 mx-auto">
                      <TrendingUp size={22} className="text-shell-text-muted" />
                    </div>
                    <p className="font-medium text-shell-text mb-1">Sin datos para {selectedYear}</p>
                  ...
                  return (
                    <tr key={agent.email} className={`border-b border-shell-border/50 hover:bg-white/[0.02] transition-colors ${!agent.isActive ? 'opacity-50' : ''}`}>
                      ...
                      {agent.monthly.map((m, i) => {
                        ...
                        return (
                          <td key={i} className="p-3 text-right">
                            <span className={`text-xs font-medium ${val > 0 ? 'text-shell-text' : 'text-shell-text-muted/30'}`}>
                              {metric === 'facturacion'
                      ...
                      <td className="p-3 text-right">
                        <span className={`text-sm font-bold ${total > 0 ? 'text-shell-accent' : 'text-shell-text-muted/30'}`}>
                          {metric === 'facturacion'
  ...
  return (
    <div className="bg-shell-surface border border-shell-border rounded-2xl p-4">
      <div className="flex items-center gap-3 mb-3">
        ...
        <h4 className="font-semibold text-sm text-shell-text">{title}</h4>
      </div>
      <div className="space-y-2">
        {items.map((item, i) => (
          <div key={i} className="flex items-center justify-between text-sm">
            ...
            <span className="font-medium text-shell-text">{item.value}</span>
          </div>
        ))}
        {items.length === 0 && <p className="text-xs text-shell-text-muted">Sin datos</p>}
      </div>
    </div>
  )
}
