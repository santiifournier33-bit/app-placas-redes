...
export default function OperationForm({ agents, onClose, onSuccess }: { agents: Agent[], onClose: () => void, onSuccess: () => void }) {
  ...
  const AgentDropdown = ({ value, onChange }: { value: string, onChange: (v: string) => void }) => (
    <select value={value} onChange={e => onChange(e.target.value)} className="w-full bg-black/20 border border-white/10 rounded-lg px-2 py-1 text-xs text-text-primary focus:outline-none focus:border-shell-accent">
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
      <div className="bg-[#0c0c14] border border-border-subtle rounded-2xl w-full max-w-4xl max-h-[90vh] flex flex-col">
        
        {/* Header */}
        <div className="flex items-center justify-between p-5 border-b border-border-subtle shrink-0">
          <h2 className="text-lg font-bold text-text-primary">Registrar Cierre</h2>
          <button onClick={onClose} className="p-2 rounded-xl hover:bg-surface-overlay-hover text-text-muted"><X size={18} /></button>
        </div>
        ...
        <div className="p-5 overflow-y-auto space-y-6">
          ...
          <div className="bg-shell-accent/5 border border-shell-accent/20 rounded-xl p-4">
            <Field label="Asociar a Reserva previa (Opcional)">
              <select value={selectedReservaId} onChange={e => setSelectedReservaId(e.target.value)} className="w-full bg-surface-1 border border-shell-accent/30 rounded-xl px-3 py-2 text-sm text-text-primary focus:outline-none focus:border-shell-accent">
            ...
            <p className="text-xs md:text-[10px] text-text-muted mt-1.5">Al seleccionar una reserva, se auto-completarán los datos de la propiedad y el valor de cierre.</p>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {/* 2. Operación Básica */}
            <div className="space-y-4">
              <h3 className="text-sm font-bold text-text-primary border-b border-border-subtle pb-2">Datos de la Operación</h3>
              ...
              <Field label="Dirección de la propiedad *">
                <input type="text" value={propertyAddress} onChange={e => setPropertyAddress(e.target.value)} className="w-full bg-surface-1 border border-border-subtle rounded-xl px-3 py-2 text-sm text-text-primary focus:outline-none focus:border-shell-accent" />
              </Field>
              <div className="grid grid-cols-2 gap-3">
                <Field label="Valor de cierre *">
                  <input type="number" inputMode="decimal" value={closeValue} onChange={e => setCloseValue(e.target.value)} className="w-full bg-surface-1 border border-border-subtle rounded-xl px-3 py-2 text-sm text-text-primary focus:outline-none focus:border-shell-accent" />
                </Field>
                <Field label="Moneda">
              ...
              {isAlqTemp && (
                <Field label="Cantidad de meses de contrato">
                  <input type="number" inputMode="decimal" value={alquilerMonths} onChange={e => setAlquilerMonths(e.target.value)} className="w-full bg-surface-1 border border-border-subtle rounded-xl px-3 py-2 text-sm text-text-primary focus:outline-none focus:border-shell-accent" />
                </Field>
              )}
              <div className="grid grid-cols-2 gap-3">
                <Field label="Fecha reserva">
                  <input type="date" value={reservationDate} onChange={e => setReservationDate(e.target.value)} className="w-full bg-surface-1 border border-border-subtle rounded-xl px-3 py-2 text-sm text-text-primary focus:outline-none focus:border-shell-accent" />
                </Field>
                <Field label="Fecha cierre *">
                  <input type="date" value={closeDate} onChange={e => setCloseDate(e.target.value)} className="w-full bg-surface-1 border border-border-subtle rounded-xl px-3 py-2 text-sm text-text-primary focus:outline-none focus:border-shell-accent" />
                </Field>
              </div>
            </div>
            ...
            <div className="space-y-4">
              <h3 className="text-sm font-bold text-text-primary border-b border-border-subtle pb-2">Honorarios y Puntas</h3>
              
              <div className="space-y-3 p-3 bg-surface-overlay rounded-xl border border-border-subtle">
                <label className="flex items-center gap-2 cursor-pointer">
                  ...
                  <span className="text-sm font-semibold text-text-primary">{isVenta ? 'Punta Vendedora' : 'Punta Locador'}</span>
                </label>
                {sideVendedor && (
                  <div className="pl-6 grid grid-cols-2 gap-3">
                    <Field label="% Honorarios">
                      <input type="number" inputMode="decimal" step="0.1" value={isVenta ? feePctVendedor : (isAlqTemp ? feePctAlquilerTemp : feePctAlquiler)} 
                        ...
                        }} className="w-full bg-black/20 border border-white/10 rounded-lg px-2 py-1 text-sm text-text-primary focus:outline-none focus:border-shell-accent" />
                    </Field>
              ...
              <div className="space-y-3 p-3 bg-surface-overlay rounded-xl border border-border-subtle">
                <label className="flex items-center gap-2 cursor-pointer">
                  ...
                  <span className="text-sm font-semibold text-text-primary">{isVenta ? 'Punta Compradora' : 'Punta Inquilino'}</span>
                </label>
                {sideComprador && (
                  <div className="pl-6 grid grid-cols-2 gap-3">
                    <Field label="% Honorarios">
              ...
              <div className="bg-surface-1 border border-border-subtle rounded-xl p-3 flex items-center justify-between">
                <div>
                  <p className="text-xs md:text-[11px] font-medium text-text-muted">Total Honorarios</p>
                  <p className="text-xl font-bold text-emerald-400">{feeCurrency} {totalFeesCalculated.toLocaleString()}</p>
                </div>
                <div>
                  <Field label="Moneda Hon.">
                    <select value={feeCurrency} onChange={e => setFeeCurrency(e.target.value)} className="bg-black/20 border border-white/10 rounded-lg px-2 py-1 text-sm text-text-primary">
                      <option value="USD">USD</option>
                      <option value="ARS">ARS</option>
                    </select>
                  </Field>
          ...
          {(sideVendedor || sideComprador) && (
            <div className="border-t border-border-subtle pt-6 space-y-6">
              <h3 className="text-sm font-bold text-text-primary">Roles y Participación</h3>
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                
                {sideVendedor && (
                  <div className="bg-surface-overlay border border-border-subtle rounded-xl p-4 space-y-4">
                    ...
                    {vendedorIsExternal ? (
                      <div className="grid grid-cols-2 gap-3">
                        <Field label="Inmobiliaria Ext."><input type="text" value={vendedorExternalName} onChange={e => setVendedorExternalName(e.target.value)} className="w-full bg-black/20 border border-white/10 rounded-lg px-2 py-1.5 text-sm text-text-primary" /></Field>
                        <Field label="Agente Ext."><input type="text" value={vendedorExternalAgent} onChange={e => setVendedorExternalAgent(e.target.value)} className="w-full bg-black/20 border border-white/10 rounded-lg px-2 py-1.5 text-sm text-text-primary" /></Field>
                      </div>
                    ) : (
                      <>
                        <div className="grid grid-cols-2 gap-3">
                          ...
                          <Field label="Banda">
                            <select value={vendedorBand} onChange={e => setVendedorBand(e.target.value)} className="w-full bg-black/20 border border-white/10 rounded-lg px-2 py-1 text-xs text-text-primary">
                        ...
                        {vendedorAgent && (
                          <div className="bg-black/30 rounded-lg p-3 space-y-3">
                            <p className="text-xs md:text-[10px] uppercase font-bold text-text-muted">Desglose de Roles</p>
                            <div className="grid grid-cols-2 gap-2 items-center">
                              <span className="text-xs text-text-primary">Lead/Prospección ({vendedorBand === 'oro' ? '30' : vendedorBand === 'plata' ? '25' : '20'}%)</span>
                              <AgentDropdown value={vendedorRoles.lead} onChange={v => setVendedorRoles(p => ({...p, lead: v}))} />
                            </div>
                            <div className="grid grid-cols-2 gap-2 items-center">
                              <span className="text-xs text-text-primary">Captación (15%)</span>
                              <AgentDropdown value={vendedorRoles.captacion} onChange={v => setVendedorRoles(p => ({...p, captacion: v}))} />
                            </div>
                            <div className="grid grid-cols-2 gap-2 items-center">
                              <span className="text-xs text-text-primary">Comercialización (15%)</span>
                ...
                {sideComprador && (
                  <div className="bg-surface-overlay border border-border-subtle rounded-xl p-4 space-y-4">
                    ...
                    {compradorIsExternal ? (
                      <div className="grid grid-cols-2 gap-3">
                        <Field label="Inmobiliaria Ext."><input type="text" value={compradorExternalName} onChange={e => setCompradorExternalName(e.target.value)} className="w-full bg-black/20 border border-white/10 rounded-lg px-2 py-1.5 text-sm text-text-primary" /></Field>
                        <Field label="Agente Ext."><input type="text" value={compradorExternalAgent} onChange={e => setCompradorExternalAgent(e.target.value)} className="w-full bg-black/20 border border-white/10 rounded-lg px-2 py-1.5 text-sm text-text-primary" /></Field>
                      </div>
                    ) : (
                      <>
                        <div className="grid grid-cols-2 gap-3">
                          ...
                          <Field label="Banda">
                            <select value={compradorBand} onChange={e => setCompradorBand(e.target.value)} className="w-full bg-black/20 border border-white/10 rounded-lg px-2 py-1 text-xs text-text-primary">
                              <option value="bronce">Bronce</option><option value="plata">Plata</option><option value="oro">Oro</option>
                            </select>
                          </Field>
                        </div>
                        {compradorAgent && (
                          <div className="bg-black/30 rounded-lg p-3 space-y-3">
                            <p className="text-xs md:text-[10px] uppercase font-bold text-text-muted">Desglose de Roles</p>
                            <div className="grid grid-cols-2 gap-2 items-center">
                              <span className="text-xs text-text-primary">Lead/Prospección ({compradorBand === 'oro' ? '30' : compradorBand === 'plata' ? '25' : '20'}%)</span>
                              <AgentDropdown value={compradorRoles.lead} onChange={v => setCompradorRoles(p => ({...p, lead: v}))} />
                            </div>
                            <div className="grid grid-cols-2 gap-2 items-center">
                              <span className="text-xs text-text-primary">Visitas/Cierre (30%)</span>
        ...
        {/* Footer */}
        <div className="flex items-center justify-end gap-3 p-5 border-t border-border-subtle shrink-0 bg-[#0c0c14]">
          <button onClick={onClose} className="px-4 py-2 rounded-xl text-sm text-text-muted hover:bg-white/5 cursor-pointer">Cancelar</button>
...
function Field({ label, children }: { label: string, children: React.ReactNode }) {
  return (
    <label className="block">
      <span className="text-xs md:text-[11px] font-medium text-text-muted mb-1.5 block">{label}</span>
      {children}
    </label>
  )
}
