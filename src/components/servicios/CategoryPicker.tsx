    ...
    <div className="relative">
      ...
      {open && (
        <>
          ...
          <div className="absolute top-full left-0 mt-1 w-full z-50 bg-[#1e1e2c] border border-border-default rounded-xl shadow-2xl max-h-72 overflow-y-auto">
              ...
              return (
                <div key={macro.id}>
                  ...
                  >
                    ...
                    <span className="text-xs text-text-muted mr-1">{subs.length}</span>
                    <ChevronRight
                      size={12}
...
function CategoryOption({ cat, selected, onSelect }: { cat: Category; selected: boolean; onSelect: () => void }) {
  return (
    <button
      type="button"
      onClick={onSelect}
      className="w-full flex items-center gap-2 pl-10 pr-3 py-2 hover:bg-surface-overlay transition-colors text-left"
    >
      <CategoryIconChip category={cat} />
      <span className={`text-sm flex-1 ${selected ? "text-text-primary" : "text-text-secondary"}`}>{cat.name}</span>
