...
export default function Dashboard({ property, onBack }: { property: any; onBack: () => void }) {
  ...
  const [preparedVideoBlob, setPreparedVideoBlob] = useState<Blob | null>(null);
  ...
  const handleVideoWhatsAppShare = async () => {
    if (preparedVideoBlob) {
      try {
        const mimeType = preparedVideoBlob.type || "video/mp4";
        const ext = mimeType.includes("webm") ? "webm" : "mp4";
        const fileName = `freire-video-${property.address ? property.address.replace(/[^a-z0-9]/gi, '_').toLowerCase() : 'propiedad'}.${ext}`;
        const file = new File([preparedVideoBlob], fileName, { type: mimeType });
        const propertyUrl = `https://www.freirepropiedades.com/p/${property.id}-prop`;
        ...
        if (navigator.canShare?.({ files: [file] })) {
          ...
          const url = URL.createObjectURL(preparedVideoBlob);
          const a = document.createElement("a");
          a.href = url;
          a.download = fileName;
          document.body.appendChild(a);
        ...
        if (e?.name !== "AbortError") {
          console.error("Error compartiendo video preparado:", e);
          showToast("No se pudo compartir. Tal vez no sea compatible.");
        }
      }
      return;
    }
    ...
    setVideoDownloadProgress(0);
    try {
      showToast("Preparando video para WhatsApp (Paso 1)...");
      const blob = await renderVideoToBlob((p) => setVideoDownloadProgress(p));
      if (!blob) throw new Error("No se pudo renderizar el video");

      setPreparedVideoBlob(blob);
      showToast("¡Listo! Tocá de nuevo el botón para compartir a WhatsApp.");
    } catch (e: any) {
      if (e?.name !== "AbortError") {
        ...
        {showVideoModal && (
          ...
          >
            <button
              ...
              className="absolute top-6 right-6 lg:top-8 lg:right-8 z-[60] bg-black/70 hover:bg-black/90 text-white rounded-full w-11 h-11 flex items-center justify-center transition-all duration-200 cursor-pointer shadow-lg border border-white/20 backdrop-blur-sm"
              title="Cerrar"
            >
                  ...
                  <div className="p-4 flex flex-col gap-2">
                    <div className="flex gap-2">
                      ...
                      <button
                        ...
                        disabled={isVideoDownloading && !preparedVideoBlob}
                        className={`btn-secondary flex-1 shadow-md text-xs disabled:opacity-50 flex items-center justify-center gap-1.5 transition-colors ${
                          preparedVideoBlob 
                            ? "!border-[#25D366] !bg-[#25D366] !text-white hover:!bg-[#20b858]"
                      ...
                      >
                        ...
                        {preparedVideoBlob ? "¡Compartir Ahora!" : "WhatsApp"}
                      </button>
                    </div>
                    <button
