...
const sheetVariants = cva(
  "fixed z-50 gap-4 bg-surface-1 text-text-primary shadow-2xl transition ease-in-out data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:duration-300 data-[state=open]:duration-500",
  {
    variants: {
      side: {
        top: "inset-x-0 top-0 border-b border-border-subtle data-[state=closed]:slide-out-to-top data-[state=open]:slide-in-from-top",
        bottom:
          "inset-x-0 bottom-0 border-t border-border-subtle rounded-t-2xl data-[state=closed]:slide-out-to-bottom data-[state=open]:slide-in-from-bottom",
        ...
        // Pantalla completa con animación SOLO fade (sin transform): un elemento
        // `position: fixed` con `transform` + apertura de teclado se rompe en iOS
...
>(({ side = "right", showClose = true, className, children, ...props }, ref) => (
  <SheetPortal>
    ...
    >
      {children}
      {showClose && (
        <SheetPrimitive.Close className="absolute right-4 top-4 rounded-md p-1.5 text-text-muted hover:text-text-primary hover:bg-surface-overlay-hover transition-colors disabled:pointer-events-none">
          <X className="size-4" />
